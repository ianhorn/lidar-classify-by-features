...
############################################################
#        RUN DATAFRAME LOCALLY, CONCURRENT (imagestorage)      #
############################################################
...
# Starts from the top, working down -- mapcache (identical specs) starts
# from the bottom instead, so the two converge toward the middle rather
# than racing over the same tiles. Plain order already matches this, no
# reordering needed.
df.shape
...
if __name__ == '__main__':
    ...
    local_path = Path(tempfile.gettempdir()) / "future_results_imagestorage.parquet"
    results_df.to_parquet(local_path)
    # _imagestorage suffix: keeps this run's summary from clobbering the
    # other branches' results files, since several may be active at once.
    upload_to_s3(local_path, S3_BUCKET, "phase2/future_results_imagestorage.parquet")
    local_path.unlink()
